...
const SettingsPage = () => {
  ...
  return (
    <div className="min-h-screen container mx-auto px-6 pt-24 max-w-5xl bg-gradient-to-br from-base-100 via-base-200 to-base-300 pb-12 select-none animate-fade-up">
      <div className="space-y-8">
        {/* Title */}
        <div className="flex flex-col gap-1.5 bg-base-100/60 p-6 rounded-2xl border border-base-300/80 backdrop-blur-md hover:border-primary/20 transition-all duration-300 select-none">
        ...
        <div className="bg-base-100/60 backdrop-blur-md rounded-3xl border border-base-300/80 p-8 space-y-6 shadow-xl hover:border-primary/10 transition-all duration-300">
          <div className="flex items-center justify-between border-b border-base-300/60 pb-3 mb-2">
            ...
            <span className="badge badge-primary badge-outline font-semibold uppercase tracking-wider">{theme}</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {THEMES.map((t) => (
              <button
                key={t}
                className={`
                  group flex flex-col items-center gap-2 p-2.5 rounded-2xl transition-all border hover-lift
                  ${theme === t ? "bg-base-200 border-primary shadow-lg ring-1 ring-primary/40 scale-[1.03]" : "bg-base-200/40 border-base-300/60 hover:bg-base-200 hover:border-base-300"}
                `}
                onClick={() => setTheme(t)}
              >
                <div
                  className="relative h-10 w-full rounded-xl overflow-hidden shadow-inner border border-base-300/40 group-hover:scale-105 transition-all"
                  data-theme={t}
                >
                ...
                <span className="text-[11px] font-bold truncate w-full text-center text-base-content/80 group-hover:text-base-content">
        ...
        <div className="bg-base-100/60 backdrop-blur-md rounded-3xl border border-base-300/80 p-8 space-y-6 shadow-xl hover:border-primary/10 transition-all duration-300">
          ...
          <div className="rounded-2xl border border-base-300 overflow-hidden bg-base-100/40 shadow-inner p-4 max-w-lg mx-auto hover-lift transition-all duration-300">
            {/* Mock Chat UI */}
            <div className="bg-base-100 rounded-xl shadow-xl overflow-hidden border border-base-300/80">
              ...
              <div className="p-4 space-y-4 min-h-[220px] max-h-[220px] overflow-y-auto bg-base-100/30 backdrop-blur-sm">
                {PREVIEW_MESSAGES.map((message) => (
                  ...
                  >
                    <div
                      className={`
                        max-w-[80%] rounded-2xl p-3.5 shadow-md backdrop-blur-sm transition-all duration-300 hover-lift border
                        ${
                          message.isSent
              ...
              <div className="p-4 border-t border-base-300 bg-base-100/60 backdrop-blur-md">
                <div className="flex gap-2">
                  <input
                    type="text"
                    className="input input-bordered input-sm flex-1 text-sm h-10 rounded-xl bg-base-100/50 outline-none focus:outline-none focus:ring-1 focus:ring-primary/40 border-base-300 focus:border-primary/60 transition-all"
                    placeholder="Type a message..."
                    value="This is a preview"
                    readOnly
                  />
                  <button className="btn btn-primary btn-sm h-10 min-h-0 rounded-xl px-3 shadow-md shadow-primary/20 hover:scale-105 active:scale-95 transition-all">
                    <Send size={16} />
                  </button>
                </div>
              </div>
