    ...
    <header className="fixed top-0 z-40 w-full bg-base-100/60 border-b border-base-300/80 backdrop-blur-md select-none transition-all duration-300 hover:border-base-300">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link to="/settings" className="btn btn-sm btn-ghost gap-2 border border-transparent hover:border-base-300 rounded-xl px-3 hover:bg-base-200/60 transition-all font-medium text-base-content/80 hover:text-base-content hover-lift">
            <Settings className="w-5 h-5 text-primary/80 group-hover:rotate-180 transition-all duration-500" />
            <span className="hidden sm:inline">Settings</span>
          </Link>
          {authUser && (
            <>
              <Link to="/profile" className="btn btn-sm btn-ghost gap-2 border border-transparent hover:border-base-300 rounded-xl px-3 hover:bg-base-200/60 transition-all font-medium text-base-content/80 hover:text-base-content hover-lift">
                <User className="size-5 text-secondary/80" />
                <span className="hidden sm:inline">Profile</span>
              </Link>
              <button
                onClick={logout}
                className="btn btn-sm btn-outline gap-2 border-base-300 hover:border-error hover:bg-error/10 hover:text-error rounded-xl px-3 transition-all font-medium hover-lift"
              >
